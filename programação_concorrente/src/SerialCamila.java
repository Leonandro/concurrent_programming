...
public class SerialCamila {
	private int k;
	private double[][] testDataset, trainDataset;
	int NUM_INSTANCES_TRAIN, NUM_INSTANCES_TEST;
	int accurates;
	
	public SerialCamila(int k, String TRAIN_FILENAME, String TEST_FILENAME, int NUM_INSTANCES_TRAIN, int NUM_INSTANCES_TEST) throws IOException {
		...
		CSVReader readCSVTrain = new CSVReader("/home/leonandro/Codes/java/programação_concorrente/datasets/diabetes.csv", 7526883);
		CSVReader readCSVTest = new CSVReader("/home/leonandro/Codes/java/programação_concorrente/datasets/test_diabetes.csv", 20);
		readCSVTrain.load();
		readCSVTest.load();
		trainDataset = readCSVTrain.getTrain();
		testDataset = readCSVTest.getTrain();
	}
